...
/*   Updated: 2024/10/01 19:33:05 by rmanzana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_puthex_fd(unsigned long long num, int fd, int uppercase)
{
	...
	result = ft_singlechar_fd(hexdigits[num % 16], fd);
	if (result == -1)
		return (-1);
	count += result;
	return (count);
